...
class NLRQ(base.LikelihoodModel):
    ...
    def varcov(self):
        ...
        vcov /= Hsq
        self.PostVarianceCalculation.Fire({"H_N":H_N, "B1":np.diag(self.kernel.B1), "sparsity":(1/(fyx**2)) , "density": fx, "vars": np.diagonal(vcov)})
        return vcov
...
def TraceVarianceCalculation(logger, info):
    logger.debug("VarCov: H_N = {} B1 = {} f_yx^-2 = {} f_x = {} vars = {}".format(info["H_N"], info["B1"], info["sparsity"], info["density"], info["vars"]))
